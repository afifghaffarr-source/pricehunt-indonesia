...
+ Camofox stealth fallback for bot detection (2026-06-15)

**Two-tier scraping:**
1. **Primary:** Playwright with `--disable-blink-features=AutomationControlled` +
   JSON extraction (fast, low ops)
2. **Fallback:** Camofox (stealth Firefox / Camoufox engine) when Playwright returns
   empty/None or raises (handles Cloudflare, DataDome, Tokopedia's own bot detection)

The fallback is **opportunistic** — if the Camofox server isn't running or the
fallback itself fails, we just return None (same as before this change). Set
`CAMOFOX_DISABLED=1` to force-disable the fallback (e.g. during local testing
without the Camofox server).
"""
...
class TokopediaCollector(BaseCollector):
    ...
    Adds Camofox stealth fallback to BaseCollector. The primary path is still
    Playwright — Camofox kicks in only when Playwright fails (returns None or
    throws). See module docstring for the two-tier strategy.
    """

    # Camofox fallback control. Set CAMOFOX_DISABLED=1 to bypass.
    # Default: enabled (auto-detects server availability per call).
    _CAMOFOX_DISABLED = os.environ.get("CAMOFOX_DISABLED", "0") == "1"
    ...
    def __init__(self):
        ...
        self.browser = None
        self.context = None
        # Track which path was used per scrape (for observability in tests)
        self.last_scrape_path: Optional[str] = None  # "playwright" or "camofox"
    ...
    async def scrape_product(self, product_url: str) -> Optional[Dict[str, Any]]:
        ...
        Two-tier: Playwright first, Camofox stealth on failure.
        Returns dict with the legacy schema (name, price, seller, etc.) for
        backward compat with the rest of the codebase.
        """
        # Tier 1: Playwright (existing flow, no change)
        try:
            result = await self._scrape_with_playwright(product_url)
            self.last_scrape_path = "playwright"
            return result
        except Exception as e:
            logger.warning(f"Playwright scrape threw: {e!r}. Trying Camofox fallback.")

        # Tier 2: Camofox stealth fallback
        if not self._CAMOFOX_DISABLED:
            try:
                result = await self._scrape_with_camofox(product_url)
                self.last_scrape_path = "camofox"
                return result
            except CamofoxError as e:
                logger.warning(f"Camofox fallback failed: {e}")
            except Exception as e:
                logger.warning(f"Camofox fallback threw: {e!r}")
        else:
            logger.debug("Camofox fallback disabled by CAMOFOX_DISABLED=1")

        return None

    async def _scrape_with_playwright(self, product_url: str) -> Optional[Dict[str, Any]]:
        """Tier 1: Playwright with anti-detection + JSON/DOM extraction."""
        ...
        try:
            logger.info(f"[playwright] Navigating to {product_url}")
            ...
            if product_data:
                ...
                logger.info(f"[playwright] Successfully extracted: {product_data.get('name', 'Unknown')}")
                return product_data
            else:
                logger.warning("[playwright] No data extracted (empty result)")
                return None

        finally:
            await page.close()

    async def _scrape_with_camofox(self, product_url: str) -> Optional[Dict[str, Any]]:
        """Tier 2: Camofox stealth browser (handles bot detection).

        Maps the TokopediaProduct dataclass from the camofox_scraper module
        into the legacy dict format used by the rest of the codebase.

        Camofox server must be running (camofox server start --background).
        If server is unreachable, CamofoxError is raised and the caller
        treats it as a failed scrape.
        """
        logger.info(f"[camofox] Scraping {product_url}")
        # 5s hydration wait — Tokopedia is a heavy SPA, this is conservative
        async with CamofoxScraper(wait_ms=5000) as scraper:
            product = await scraper.scrape_product(product_url)
            data = self._tokopedia_product_to_dict(product)
            logger.info(
                f"[camofox] Extracted: {data.get('name', 'Unknown')!r} "
                f"@ Rp{data.get('price', 0):,}"
            )
            return data

    @staticmethod
    def _tokopedia_product_to_dict(p: TokopediaProduct) -> Dict[str, Any]:
        """Map the TokopediaProduct dataclass to the legacy dict format.

        Legacy format (kept for backward compat with ingestion_client,
        cron_scraper, ingestion API):
            {
                'name': str,
                'price': int,           # raw rupiah
                'price_display': str,   # formatted "Rp14.980.000"
                'currency': 'IDR',
                'rating': float | None,  # score (4.5)
                'sold': int | None,      # sold count
                'seller': str | None,
                'url': str,
        ...
        return {
            'name': p.title,
            'price': p.price_idr,
            'price_display': f"Rp{p.price_idr:,}" if p.price_idr else None,
            'currency': 'IDR',
            'rating': None,  # Camofox schema extracts rating count, not score
            'sold': p.sold_count,
...
async def main():
    ...
    print(f"\n🧪 Testing Tokopedia 2026 Collector (with Camofox fallback)\n{'='*50}\n")
    ...
    if result:
        ...
        print(f"🛤️  Path: {collector.last_scrape_path}")
    else:
