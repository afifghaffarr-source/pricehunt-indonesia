...
function addCorsHeaders(response: NextResponse, origin: string | null) {
  if (origin && isAllowedOrigin(origin)) {
    ...
    response.headers.set('Access-Control-Allow-Headers', 'Content-Type, Authorization, X-CSRF-Token, X-CSRF, X-Requested-With');
 ...
 * 5. Anything else: 403.
 */
function checkCsrf(request: NextRequest, pathname: string): NextResponse | null {
