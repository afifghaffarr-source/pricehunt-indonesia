...
- `docs/AUDIT_2026-06-14.md` (in progress): independent senior audit, 2-batch scope.
  ...
  - **A-002 migration Batch 1 (applied, `391981a`):** new `src/lib/ingestion/adapter.ts` (`toPriceView`/`toPriceViews`/`isOfferInStock`) + `api/deals/route.ts` switched from `prices` to `offers`. Component-facing shape preserved via adapter — no UI changes. Live data: deals now sees Tokopedia offers that were previously invisible. A-009 + A-006 also fixed earlier (commits `93f1d66`, `c52d4dd`).
